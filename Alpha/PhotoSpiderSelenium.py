...
def spider(item):  # item : 元素為[str key, int count]的雙重陣列
    ...
    driver = webdriver.Chrome()  # 開一個webdriver
    ...
    time.sleep(0.5)  # 要等瀏覽器處理button點擊
    ...
    # with concurrent.futures.ThreadPoolExecutor(max_workers=60) as executor:
        # executor.map(functools.partial(getImage, keyword = keyword, driver = driver), imageLinks, range(count))
        # for i in range(count):
        #     executor.submit(getImage, imageLinks[i], i, keyword, driver)
    # 方法二：運用for迴圈的方式爬
...
# inputItems = 'apple: 2, car sign: 12, banana: 7'
